                    ...
                    <form class="space-y-4 md:space-y-6" method="post">
                        ...
                        <div>
                            ...
                            <select name="manage" id="manage" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                <?php
                                while ($row = mysqli_fetch_assoc($selectManageSql)) {
                                    echo "<option value=" . $row['id'] . ">" . $row['name'] . "</option>";
                                }
                                ?>
                            </select>
                            <span class="field_error" id="manage_error"></span>
                            <!-- <input type="text" name="manage" id="manage" placeholder="123456789" class="" required=""> -->
                        </div>
                        <div>
                        ...
                        <button onclick="admin_register()" class="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                            <!-- Create an account -->
                            <?php echo $translations[$language]['create_account']; ?>
                        </button>
                        <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                            <!-- Already have an account?  -->
                            <?php echo $translations[$language]['already_have_account']; ?>
                            <a href="./login.php" class="font-medium text-primary-600 hover:underline dark:text-primary-500">
                                <!-- Login here -->
                                <?php echo $translations[$language]['login_here']; ?>
                            </a>
                        </p>
                    </form>
                </div>
